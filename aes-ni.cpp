...
void aes128_dec_impl<10>(__m128i &m0, __m128i &m1, __m128i &m2, __m128i &m3,
                         const __m128i *keys) {
    m0 = _mm_xor_si128(m0, keys[10]);
    m1 = _mm_xor_si128(m1, keys[10]);
    m2 = _mm_xor_si128(m2, keys[10]);
    m3 = _mm_xor_si128(m3, keys[10]);
    aes128_dec_impl<11>(m0, m1, m2, m3, keys);
}

template <>
void aes128_dec_impl<20>(__m128i &m0, __m128i &m1, __m128i &m2, __m128i &m3,
                         const __m128i *keys) {
    m0 = _mm_aesdeclast_si128(m0, keys[0]);
    m1 = _mm_aesdeclast_si128(m1, keys[0]);
    m2 = _mm_aesdeclast_si128(m2, keys[0]);
    m3 = _mm_aesdeclast_si128(m3, keys[0]);
}
} // namespace internal
...
void AES128::dec(uint8_t *out, const uint8_t *in, const size_t iter_n) const {
    ...
    for(size_t i = 0; i < iter_q4; i++) {
        ...
        __m128i m2 = _mm_loadu_si128(p_in4 + 2);
        __m128i m3 = _mm_loadu_si128(p_in4 + 3);
        internal::aes128_dec_impl<10>(m0, m1, m2, m3, expanded_keys_);
        auto *p_out4 = p_out + 4 * i;
        _mm_storeu_si128(p_out4, m0);
        _mm_storeu_si128(p_out4 + 1, m1);
        _mm_storeu_si128(p_out4 + 2, m2);
